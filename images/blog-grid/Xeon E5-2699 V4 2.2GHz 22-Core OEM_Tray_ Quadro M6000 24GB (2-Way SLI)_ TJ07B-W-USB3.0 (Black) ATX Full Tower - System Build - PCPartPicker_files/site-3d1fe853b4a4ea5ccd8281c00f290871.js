...
!function(e){var a=function(a,n){function i(){var e=a.val();e!=v&&(v=e,e.length>=s.minimumSearchLength?t(e):0==e.length&&t(""))}function t(e){l=void 0,s.searchCallback(e)}function c(){a.addClass("active_search"),a.removeClass("inactive_search"),a.val("")}function r(){a.addClass("inactive_search"),a.removeClass("active_search"),a.val(s.innerText)}a=e(a);var s=e.extend({},e.fn.livesearch.defaults,n),l=void 0,v=a.val();a.empty(),a.bind("keyup",function(){void 0!=l&&clearTimeout(l),l=setTimeout(i,s.queryDelay)}),this.DoSearch=i,""==a.val()||a.val()==s.innerText?r():c(),a.focus(function(){a.hasClass("inactive_search")&&c()}),a.blur(function(){""==a.val()&&r()})};e.fn.livesearch=function(n){this.each(function(){var i=e(this);if(!i.data("livesearch")){var t=new a(this,n);i.data("livesearch",t)}})},e.fn.livesearch.defaults={queryDelay:250,innerText:"Search",minimumSearchLength:3}}(jQuery);
...
// Code below Copyright 2015 PCPartPicker, LLC
...
function string_to_set(s) {
	return array_to_set(string_to_array(s))
}
...
function set_to_string(set) {
	return set_to_array(set).join(',');
}

function filter_by_set(a, set) {
	var i = a.length;
	while (i--) {
		if (!set[a[i]]) {
			a.splice(i, 1);
...
function filter_get_page() {
	var pagestr = $.bbq.getState()['page'];
	if (pagestr && pagestr != '')
		return parseInt(pagestr);
	return 1;
}

function filter_set_sort(sortlist) {
	var s = '';
...
function filter_get_sort() {
	var state = $.bbq.getState();
	var sortlist = [];
	if (state['sort']) {
		var sortsplit = state['sort'].split(',');
		for (var i in sortsplit)
			if (sortsplit[i][0] == 'd')
				sortlist.push([parseInt(sortsplit[i].substr(1)), 1]);
			else
				sortlist.push([parseInt(sortsplit[i].substr(1)), 0]);
	}
	return sortlist;
}
function filter_set_update_state(name) {
	var state = $.bbq.getState();
	var values = [];
	var nlen = name.length;
	$('#' + name + '_set li input.filteritem').each(function (index, element) {
		if ($(this).is(':checked')) {
			var id = $(this).attr('id').substr(nlen);
			values.push(id);
		}
	});
	if (state['page'])
...
function filter_set_update_dualslide_state(name, values) {
	var state = $.bbq.getState();
	if (state['page'])
		state['page'] = 1;
	if (values.length) {
	...
	for (var i in datafilters) {
		...
		if (datafilter.style == 'list') {
			...
			for (var j in datafilter.items) {
				...
				$('#' + datafilter.prefix + datafilter.items[j].id).prop('checked', is_checked);
				if (is_checked)
					$('#li_' + datafilter.prefix + datafilter.items[j].id).show();
				else if (datafilter.items[j].hide && !$('#li_' + datafilter.prefix + datafilter.items[j].id).is(':visible'))
					needs_show_all = true;
			}
			...
			datafilter.set_slider_values($.map(string_to_array(state[datafilter.prefix]), function (x, i) { return parseInt(x) }));
		}
	}
}
function register_filter_handlers(datafilters ) {
	$('#compatibility_enabled').click(function () {
	...
	$.each(datafilters, function (i, datafilter) {
		if (datafilter.style == 'list') {
			$('#' + datafilter.prefix + '_all').click(function () {
				if (!$(this).is(':checked'))
					return false;
				else {
					...
					filter_set_update_state(datafilter.prefix);
					return true;
				}
			});
			...
			configure_datafilter_slider(datafilter, filter_set_update_dualslide_state);
			$('#filter_slide_' + datafilter.prefix).parents('.portlet, .portlet-dfh').first().on('showing', function () {
				$(this).find(".portlet-content").show();
				datafilter.update_slider_labels();
				$(this).find(".portlet-content").hide();
			});
		}
		else if (datafilter.style == 'priceslide') {
			configure_datafilter_priceslider(datafilter, filter_set_update_dualslide_state);
			$('#filter_slide_' + datafilter.prefix).parents('.portlet, .portlet-dfh').first().on('showing', function () {
				$(this).find(".portlet-content").show();
				datafilter.update_slider_labels();
				$(this).find(".portlet-content").hide();
...
function make_currency_formatter(ccstr) {
	return function(x) { return ccstr + x; };
}
function scale_linear(x) { return x; }
function scale_log(x) { return parseInt(Math.log(x) * 100); }
function scale_log2(x) { return parseInt(Math.log2(x) * 100); }
function format_num(x) { return parseInt(x); }
function format_db(x) { return parseInt(x) + ' dB'; }
function format_ms(x) { return parseInt(x) + ' ms'; }
function format_brightness(x) { return parseInt(x) + ' cd/m&sup2;'; }
function format_optical_speed(x) { return parseInt(x) + 'X'; }
function format_ohms(x) { return format_xb(x / 1000, 1, 1000, ' &#8486;'); }
function format_mb(x) { return format_xb(x, 1024*1024, 1024, 'B'); }
function format_mbi(x) { return format_xb(x, 1000*1000, 1000, 'B'); }
function format_hz(x) { return format_xb(x, 1, 1000, 'Hz'); }
function format_mhz(x) { return format_xb(x, 1000*1000, 1000, 'Hz'); }
function format_mm(x) { return parseInt(x) + 'mm'; }
function format_w(x) { return parseInt(x) + 'W'; }
function format_mw(x) { return format_mxb(x, 'W'); }
function format_mv(x) { return format_mxb(x, 'V'); }
function format_cas(x) {
	if (x % 10 == 0)
		return (x / 10);
	var _x = x / 10.0;
	return _x.toFixed(1);
...
function format_xb(x, scale, divisor, postfix) {
	...
	for (var i = 0; i < prefix.length; ++i) {
		if ((x / d) < divisor) {
			if ((x % d) == 0)
				return (x / d) + prefix[i] + postfix;
			var _x = (x * 1.0 / d);
			if ((x*2 % d) == 0) 
				return _x.toFixed(1) + prefix[i] + postfix;
			if ((x*4 % d) == 0) 
				return _x.toFixed(2) + prefix[i] + postfix;
			return _x.toFixed(1) + prefix[i] + postfix;
		}
		d = d * divisor;
	}
...
function format_display_inches(x) {
	if (x % 100 == 0) 
		return (x / 100) + '"';
	var _x = (x * 1.0 / 100);
	if (x % 10 == 0) 
...
function format_weight_grams(x) {
	var _x = x / 453.592;
	return _x.toFixed(1) + ' lbs';
}
function format_ml(x) { return x + 'mL'; }
function format_price(ccstr, p) { var d = parseInt(p/100); var c = p - (d * 100); if (c < 10) return ccstr + d + '.0' + c; return ccstr + d + '.' + c; }
function format_price_cc(p) { var d = parseInt(p/100); var c = p - (d * 100); if (c < 10) return global_country_sign + d + '.0' + c; return global_country_sign + d + '.' + c; }
function format_prefix_cc(p) { return global_country_sign; }
function format_price_whole(p) { return parseInt(p); }

function parse_int(x) { return parseInt(x); }
function ds_parse_div1024(x) { return parseInt(x) * 1024; }
function ds_parse_div1000(x) { return parseInt(parseFloat(x) * 1000); }
function ds_parse_div454(x) { return parseInt(parseFloat(x) * 454); }
function ds_parse_div100(x) { return parseInt(parseFloat(x) * 100); }
...
function ds_format_div1024(x) { return _ds_format_divd(x, 1024); }
function ds_format_div1000(x) { return _ds_format_divd(x, 1000); }
function ds_format_div454(x) { return _ds_format_divd(x, 454); }
function ds_format_div100(x) { return _ds_format_divd(x, 100); }

function configure_datafilter_slider(datafilter, stop_callback) {
	if (!datafilter.scaler)
		datafilter.scaler = scale_linear;
	function rescale_values(a) {
		return $.map(a, function(value, i) {
			return datafilter.scaler(value);
		});
	}
	var values = $.map(datafilter.items, function(item, i) {
		return item.id;
	});
	var scaled_values = rescale_values(values);
	var scaled_min = scaled_values[0];
	var scaled_max = scaled_values[scaled_values.length - 1];
	var lval = values[0];
	var rval = values[values.length - 1];
	var scaled_lval = scaled_min;
	var scaled_rval = scaled_max;
	var llabel = $('#filter_slide_left_' + datafilter.prefix);
	var llabela = $('#filter_slide_left_' + datafilter.prefix + ' a');
	var rlabel = $('#filter_slide_right_' + datafilter.prefix);
	var rlabela = $('#filter_slide_right_' + datafilter.prefix + ' a');
	var slider_obj = $('#filter_slide_' + datafilter.prefix).slider({
		range: true,
		min: scaled_min,
		max: scaled_max,
		values: [scaled_lval, scaled_rval],
		slide: function(event, ui) {
			scaled_lval = get_nearest(scaled_values, ui.values[0]);
			scaled_rval = get_nearest(scaled_values, ui.values[1]);
			lval = values[$.inArray(scaled_lval, scaled_values)];
			rval = values[$.inArray(scaled_rval, scaled_values)];
			update_slider();
			return false;
		},
		stop: function() {
	...
	function on_stop() {
		if (scaled_lval == scaled_min && scaled_rval == scaled_max) {
			stop_callback(datafilter.prefix, []);
		} else if (scaled_lval == scaled_rval) {
			stop_callback(datafilter.prefix, [lval]);
		} else {
			stop_callback(datafilter.prefix, [lval, rval]);
		}
	}
	function bind_label(label, pos) {
		var _ll = label.children('a');
		var _lt = label.children('div.labelbox-input');
		var _lti = _lt.children('input');
		function commit_llabel(a) {
			_lt.hide();
			var old_vals = datafilter.get_slider_values();
			var new_val = datafilter.parser(_lti.val());
			if (!isNaN(new_val)) {
				if (new_val < values[0])
					new_val = values[0];
				if (new_val > values[values.length - 1])
					new_val = values[values.length - 1];
				old_vals[pos] = new_val;
				datafilter.set_slider_values(old_vals);
				on_stop();
			}
			_ll.show();
			reposition_labels();
		...
		_ll.click(function() {
			_ll.hide();
			var old_vals = datafilter.get_slider_values();
			_lti.val(datafilter.ti_formatter(old_vals[pos]));
			_lt.show();
			_lti.focus().select();
			return false;
		});
	...
	function update_slider() {
		slider_obj.slider('option', 'values', [scaled_lval, scaled_rval]);
		if (scaled_lval == scaled_rval && scaled_rval == scaled_max) {
			...
			slider_obj.slider('option', 'value', 0, scaled_lval);
		}
		reposition_labels();
	}
	function reposition_labels() {
		var barwidth = $('#filter_slide_labelbox_' + datafilter.prefix).width() - 20;
		var calc_loffset = parseInt(barwidth * (scaled_lval - scaled_min) / (scaled_max - scaled_min));
		var calc_roffset = parseInt(barwidth * (scaled_rval - scaled_min) / (scaled_max - scaled_min));
		rlabel.show();
		rlabela.html(datafilter.formatter(rval));
		llabela.html(datafilter.formatter(lval));
		var lmin = Math.max(-10, -llabel.width() / 2);
		var rmax = Math.min(barwidth - rlabel.width() + 10, barwidth - rlabel.width() / 2);
		var lpos = Math.max(calc_loffset - llabel.width() / 2, lmin);
		var rpos = Math.min(calc_roffset - rlabel.width() / 2, rmax);
		if (scaled_lval == scaled_rval) {
			rlabel.hide();
			lpos = Math.min(lpos, rmax)
		} else {
			var overlap = lpos + llabel.width() - rpos + 10;
			if (overlap > 0) {
				lpos -= overlap / 2;
				rpos += (overlap + 1) / 2;
				if (rpos > rmax) {
		...
		llabel.css({left: lpos + 10});
		rlabel.css({left: rpos + 10});
	}
	function set_values(new_lval, new_rval) {
		var added = false;
		if ($.inArray(new_lval, values) < 0) {
			values.push(new_lval);
			added = true;
		}
		if ($.inArray(new_rval, values) < 0) {
			values.push(new_rval);
			added = true;
		}
		if (added) {
			values.sort(function(a,b){return a - b;});
			scaled_values = rescale_values(values);
			scaled_min = scaled_values[0];
			scaled_max = scaled_values[scaled_values.length - 1];
			slider_obj.slider('option', 'min', scaled_min);
			slider_obj.slider('option', 'max', scaled_max);
		}
		lval = new_lval;
		rval = new_rval;
		scaled_lval = scaled_values[$.inArray(lval, values)];
		scaled_rval = scaled_values[$.inArray(rval, values)];
		update_slider();
	}
	datafilter.update_slider_labels = reposition_labels;
	datafilter.set_slider_values = function(new_values) {
		if (new_values.length == 0) {
			set_values(values[0], values[values.length - 1])
		} else if (new_values.length == 1) {
			set_values(new_values[0], new_values[0]);
		} else {
			set_values(new_values[0], new_values[1]);
		}
	}
	datafilter.get_slider_values = function() { return [lval, rval]; }
	datafilter.slider_at_default_range = function() { return (scaled_lval == scaled_min) && (scaled_rval == scaled_max); }
}

function set_datafilter_minmax(datafilters, prefix, minval, maxval) {
	for (var i in datafilters) {
		if (datafilters[i].prefix == prefix) {
			datafilters[i].minval = minval;
			datafilters[i].maxval = maxval;
			return;
		}
	}
}

function configure_datafilter_priceslider(datafilter, stop_callback) {
	var minval = datafilter.minval;
	var maxval = datafilter.maxval;
	var lprice = datafilter.minval;
	var rprice = datafilter.maxval;
	var lpos = 0;
	var rpos = 1000;
	var llabel = $('#filter_slide_left_' + datafilter.prefix);
	var llabela = $('#filter_slide_left_' + datafilter.prefix + ' a');
	...
	var slider_obj = $('#filter_slide_' + datafilter.prefix).slider({
		...
		slide: function(event, ui) {
			if (ui.values[0] != lpos) {
				lpos = ui.values[0];
				lprice = pos_to_price(ui.values[0]);
			}
			if (ui.values[1] != rpos) {
				rpos = ui.values[1];
				rprice = pos_to_price(ui.values[1]);
			}
			update_slider(lprice, rprice);
			return false;
		},
	...
	function on_stop() {
		if (lprice == minval && rprice == maxval) {
			stop_callback(datafilter.prefix, []);
		} else if (lprice == rprice) {
			stop_callback(datafilter.prefix, [lprice]);
		} else {
			stop_callback(datafilter.prefix, [lprice, rprice]);
		}
	}
	function get_slide_values() {
		return $('#filter_slide_' + datafilter.prefix).slider('option', 'values');
	}
	function bound_price(price, default_price) {
	...
	function get_log_const() {
		return Math.sqrt(maxval / 1000);
	}
	function price_to_pos(price) {
		var r = price * 1.0 / maxval;
		var rlog = Math.log((Math.pow(get_log_const(), 1) - 1) * r + 1) / Math.log(get_log_const());
		return parseInt(rlog * 1000);
	}
	function pos_to_price(pos) {
		var rpos = (pos / 1000.0);
		var rprice = (Math.pow(get_log_const(), rpos) - 1) / (Math.pow(get_log_const(), 1) - 1);
		return parseInt(rprice * maxval);
	}
	function bind_label(label, pos) {
		var _ll = label.children('a');
		var _lt = label.children('div.labelbox-input');
		...
		function commit_llabel(a) {
			_lt.hide();
			var old_prices = [lprice, rprice];
			var new_price = parseInt(_lti.val()) * 100;
			old_prices[pos] = bound_price(new_price, [minval, maxval][pos]);
			datafilter.set_slider_values(old_prices);
			on_stop();
			_ll.show();
			reposition_labels();
		}
		_ll.click(function() {
			_ll.hide();
			var values = get_slide_values();
			_lti.val(parseInt(pos_to_price(values[pos]) / 100));
			_lt.show();
			_lti.focus().select();
			return false;
		});
	...
	bind_label(rlabel, 1);
	bind_label(llabel, 0);
	function update_slider(new_lprice, new_rprice) {
		lpos = price_to_pos(new_lprice);
		rpos = price_to_pos(new_rprice);
		slider_obj.slider('option', 'values', [lpos, rpos]);
		if (new_lprice == new_rprice && new_rprice == maxval) {
			/* hack to allow right slider to back off - slider is set so on overlap last moved is the active one.  but if both are all the way 
			   to the right, and the right is the active slider, then you're stuck.  in that case, force the left to be the active slider by
			   setting it separately and last. */
			slider_obj.slider('option', 'value', 0, lpos);
		}
		reposition_labels();
	}
	function reposition_labels() {
		...
		var rval = values[1];
		var barwidth = $('#filter_slide_labelbox_' + datafilter.prefix).width() - 20;
		var calc_loffset = parseInt(barwidth * lval / 1000);
		var calc_roffset = parseInt(barwidth * rval / 1000);
		llabela.html(datafilter.formatter_prefix() + datafilter.formatter(lprice / 100));
		rlabel.show();
		rlabela.html(datafilter.formatter_prefix() + datafilter.formatter(rprice / 100));
		var lmin = Math.max(-10, -llabel.width() / 2);
		var rmax = Math.min(barwidth - rlabel.width() + 10, barwidth - rlabel.width() / 2);
		var lx = Math.max(calc_loffset - llabel.width() / 2, lmin);
		var rx = Math.min(calc_roffset - rlabel.width() / 2, rmax);
		if (lval == rval) {
			rlabel.hide();
			lx = Math.min(lx, rmax)
		} else {
			var overlap = lx + llabel.width() - rx + 10;
			if (overlap > 0) {
				lx -= overlap / 2;
				rx += (overlap + 1) / 2;
				if (rx > rmax) {
		...
		llabel.css({left: lx + 10});
		rlabel.css({left: rx + 10});
	}
	datafilter.update_slider_labels = reposition_labels;
	datafilter.set_slider_values = function(new_prices) {
		if (new_prices.length == 0) {
			lprice = minval;
			rprice = maxval;
			update_slider(minval, maxval);
		} else if (new_prices.length == 1) {
			lprice = bound_price(new_prices[0], minval);
			rprice = bound_price(new_prices[0], maxval);
			update_slider(lprice, rprice);
		} else {
			lprice = bound_price(new_prices[0], minval);
			rprice = bound_price(new_prices[1], maxval);
			if (lprice > rprice)
				lprice = rprice;
			update_slider(lprice, rprice);
		}
	}
	datafilter.slider_at_default_range = function() { return (lval == minval) && (rval == maxval); }
}

function async_load_partlist_price_history(selector, currency_str, request_data, for_mobile, legend_selector) {
	$.getJSON('/qapi/partlist/pricehistory/', request_data, function(data) {
		if (data.success) {
			var plots = [];
			...
			var legend;
			if (legend_selector !== undefined) {
				legend = {
					show: true,
					noColumns: for_mobile ? 1 : 2,
					position: "nw",
					labelFormatter: function(label, series) {
						return '<div class="partlist_price_history_label">' + label + '</div>';
					},
					sorted: 'reverse',
			...
			$.plot($(selector), plots,
				{
					...
					xaxis: {
						mode: 'time',
						minTickSize: [1, "day"],
						tickSize: for_mobile ? [1, "month"] : [7, "day"]
					},
					yaxis: {
		...
		$(selector).html('Loading chart data failed: ' + textStatus);
	});
}

function async_load_partlist_price_history_v2(selector, currency_str, request_data, name_map, upps, for_mobile, legend_selector) {
	$.getJSON('/qapi/partlist/pricehistory_v2/', request_data, function(data) {
		if (data.success) {
			var plots = [];
			for (var i in data.plots) {
				var plot_data = data.plots[i];
			...
			update_flot_legend(plot, legend_selector);
		}
		else {
			$(selector).html(data.error);
		}
...
function cx_login_to_vote() { window.location.assign(window.location.protocol + "//" + window.location.host + '/accounts/login/?next=' + window.location.pathname + window.location.search + window.location.hash); }
...
function cx_vote(root_id, comment_id, amount) {
	...
	points = points + (downvoted ? 1 : 0) + (upvoted ? -1 : 0) /* return points to our prevote total */
	...
	$.post('/comments/vote/', { comment_root: root_id, comment_id: comment_id, amount: amount }, function (data) {
...
function cx_edit_toggle(root_id, parent_id, comment_id) {
	...
	if (!crdiv.is(':visible')) {
		$.post('/comments/view/', { comment_id: comment_id }, function (data) {
			if (data.success) {
...
function cx_submit_edit(s, comment_id) {
	...
	if (is_blank) {
		...
		$form_error.hide();
		$.post('/comments/post/', $(s.form).serialize(), null, "json")
			.done(function (data) {
				if (data.success) {
					$('#cx' + comment_id + '_item').replaceWith(data.content);
...
function cx_delete_confirm(comment_id) {
	$.post('/comments/delete/', { comment_id: comment_id }, null, "json")
		.done(function (data) {
			if (data.success)
...
function cx_undelete(comment_id) {
	$.post('/comments/undelete/', { comment_id: comment_id }, function (data) {
		if (data.success)
			$('#cx' + comment_id + '_undelete').hide();
	}, "json");
}
function cx_mark_read(comment_id, comment_read) {
	var comment_item = $('#cx' + comment_id + '_item');
	var comment_mark_unread = $('#cx' + comment_id + '_mark_unread');
	if ((comment_read && !comment_item.hasClass('comment-unread')) || (!comment_read && comment_item.hasClass('comment-unread')))
		return;
	$.post('/comments/mark/read/', { comment_id: comment_id, comment_read: comment_read }, function (data) {
		if (data.success) {
			if (comment_read) {
				comment_item.removeClass('comment-unread');
				comment_mark_unread.show();
			}
			else {
				comment_item.addClass('comment-unread');
				comment_mark_unread.hide();
			}
		}
	}, "json");
}
...
function mx_submit_reply(s, message_id) {
	...
	if (is_blank) {
		...
		$form_error.hide();
		$save_button.prop('disabled', true);
		$.post('/comments/message/reply/', $(s.form).serialize(), null, "json")
			.done(function (data) {
				if (data.success) {
...
function mx_mark_read(message_id, message_read) {
	var mitem = $('#mx' + message_id + '_item');
	var munread = $('#mx' + message_id + '_mark_unread');
	if ((message_read && !mitem.hasClass('comment-unread')) || (!message_read && mitem.hasClass('comment-unread')))
		return;
	$.post('/comments/message/mark/read/', { message_id: message_id, message_read: message_read }, function (data) {
		if (data.success) {
			if (message_read) {
				mitem.removeClass('comment-unread');
				munread.show();
			}
			else {
				mitem.addClass('comment-unread');
				munread.hide();
			}
		}
	}, "json");
...
function clear_search_result(results_element) {
	$(results_element).hide().html("");
	num_search_items = 0;
	search_selection_index = -1;
	selection_urls = [];
}
function on_search(search_term, results_element, show_add_link) {
	search_id++;
	$.getJSON('/search/', { live: 1, q: search_term, qid: search_id }, function (data) {
		if (data.success && data.qid == search_id) {
			if (data.results && data.results.length > 0) {
				var t = "<ul>";
				update_search_selection_index(-1);
				selection_urls = [];
				num_search_items = data.results.length;
				for (var i = 0; i < data.results.length; ++i) {
					var viewurl = '/product/' + data.results[i].tag + '/' + data.results[i].slug;
					selection_urls.push(viewurl);
					t += '<li id="search_selection_' + i + '">';
					t += '<a href="' + viewurl + '">';
					tstr = data.results[i].ht_full + ' (' + data.results[i].mpn + ')';
					var j = tstr.toLowerCase().indexOf(search_term.toLowerCase());
					if (j >= 0) {
						tstr = tstr.substr(0, j) + '<span class="search-highlight">' + tstr.substr(j, search_term.length) + '</span>' + tstr.substr(j + search_term.length);
					}
					t += tstr + '</a>';
					if (show_add_link)
						t += '<a href="#' + data.results[i].tag + '" class="pp_add_part btn-mds">Add</a>';
					t += '</li>';
				}
				t += "</ul>";
				$(results_element).html(t);
				$(results_element).show();
			}
			else
				clear_search_result(results_element);
		}
	});
}
function init_live_search(init_element, results_element, show_add_link) {
	init_element = (typeof init_element === "undefined") ? "#search_q" : init_element;
	results_element = (typeof results_element === "undefined") ? "#search_result" : results_element;
	$(init_element).livesearch({
		searchCallback: function (search_term) { return on_search(search_term, results_element, show_add_link); },
		innerText: "",
		queryDelay: 100,
		minimumSearchLength: 1
	});
	$(init_element).keydown(function (e) {
		if (e.keyCode == 38) { /* up */
			if (search_selection_index >= 0)
		...
		setTimeout(function () { clear_search_result(); }, 250);
	});
}

function FilteredFetchList(category_search_selector, category_sort_selector, datafilters, fetch_url, extra, fetch_callback)
{
	var self = this;
	...
	this.update = function(q) {
		var state = $.bbq.getState();
		for (var i in extra) { state[i] = extra[i]; }
		var curr_search_term = $(category_search_selector).val();
		if (q == null || curr_search_term != search_term) {
			if (q != null)
				search_term = curr_search_term;
			state['search'] = search_term;
			$.getJSON(fetch_url, state, function(data) {
				if (data.success) {
					fetch_callback(data.result);
				}
				else {
	...
	this.hashchange = function() {
		var state = $.bbq.getState();
		$('ul.tabbed-nav li a').each(function(index, element) { $(this).fragment('href', state, 2); });
		update_filters_from_state(state, datafilters);
		if (category_sort_selector) {
			if (state['sort'])
	...
	if (category_search_selector) {
		$(category_search_selector).keydown(function(e){
			if (e.keyCode == 27) { /* escape */
				$(category_search_selector).blur();
			}
		}).livesearch({
			searchCallback: function(q) { return self.update(q); },
			innerText: "",
			queryDelay: 100,
			minimumSearchLength: 1
		});
	...
	if (category_sort_selector) {
		$(category_sort_selector).change(function() {
			var val = $(category_sort_selector).val();
			var index = parseInt(val.substring(1));
			var ascending = (val.substring(0, 1) == 'a') ? 0: 1;
			filter_set_sort([[index, ascending]]);
		});
	}
}

function pp_common_ajax_redirect(url, data) {
	$.ajax({
		type: "POST",
		url: url,
		data: data,
...
function pp_common_ajax_callback(url, data, callback_func) {
	$.ajax({
		type: "POST",
		url: url,
		data: data,
...
function pp_topic_delete(topic_id) { pp_common_ajax_redirect('/forums/topic/delete/', { topic_id: topic_id }); }
function pp_topic_undelete(topic_id) { pp_common_ajax_redirect('/forums/topic/undelete/', { topic_id: topic_id }); }
function pp_favorite_add(tag) { pp_common_ajax_redirect('/qapi/favorite/add/', { tag: tag }); }
function pp_favorite_delete(tag, user) { pp_common_ajax_redirect('/qapi/favorite/delete/', { tag: tag, user: user }); }
function pp_favorite_delete_cb(xid, user, callback_func) { pp_common_ajax_redirect('/qapi/favorite/delete_by_id/', { xid: xid, user: user}, callback_func); }
function pp_inventory_add(tag) { pp_common_ajax_redirect('/qapi/inventory/add/', { tag: tag }); }
function pp_inventory_delete_cb(xid, user, callback_func) { pp_common_ajax_callback('/qapi/inventory/delete/', { xid: xid, user: user}, callback_func); }
function pp_report_object(linkage_type, linkage_id, reason, message, success_callback) {
	$.ajax({
		type: "POST",
...
function pp_update_carousel(container) {
	$(container).owlCarousel({
		...
		navigation: true,
		navigationText: [
			"<a class='carousel-left-arrow'></a>",
			"<a class='carousel-right-arrow'></a>"
		],
	});
}
function pp_reorder_partlists_common(reorder_url, permalink_tags) {
	$.ajax({
		type: 'POST',
		url: reorder_url,
		data: {
			permalink_tags: permalink_tags
		},
		dataType: "json"
	}).done(function(data, text_status, xhr) {
		if (!data.success)
		...
		$(container).html('An unknown error occurred.');
	});
}
function pp_reorder_saved_part_lists(permalink_tags) {
	pp_reorder_partlists_common('/qapi/user/saved/reorder/', permalink_tags);
}

function update_flot_legend(plot, container) {
	var flot_data = plot.getData();
	for(var i = 0; i < flot_data.length; ++i)
		$(container).append('<div class="price-history-legend-item"><span class="price-history-legend-color" style="background-color:' + flot_data[i].color + ';"></span>' + flot_data[i].label + '</div>');
}

function update_flot_legend_responsive(plot, container) {
	var flot_data = plot.getData();
	for(var i = 0; i < flot_data.length; ++i)
		$(container).append('<li class="xs-inline-block xs-mr3 xs-mb1"><span class="legend-color" style="background-color:' + flot_data[i].color + ';"></span>' + flot_data[i].label + '</li>')
        //<div class="price-history-legend-item"><span class="price-history-legend-color" style="background-color:' + flot_data[i].color + ';"></span>' + flot_data[i].label + '</div>');
}

function pp_comment_root_subscribe_toggle(root_id, container) { 
	$.ajax({
		type: "POST",
		url: '/comments/root/subscribe_toggle/',
...
function pp_comment_root_subscribe_delete(id, container) { 
	$.ajax({
		type: "POST",
		url: '/comments/root/subscribe_delete/',
		data: { id: id },
		dataType: "json"
	}).done(function(data, text_status, xhr) {
		if (data.success)
...
function pp_post_wait(url, data, complete_func, error_func) {
    ...
    function _pp_post(url, data, complete_func, error_func) {
        $.ajax({
            ...
            _pp_post_handle_response(data);
        }).fail(function(xhr, text_status, error_thrown) {
            error_func('An unknown error occurred.');
        });
    }
    ...
    run_confirm_dialog('Confirm Account Deletion', 'Are you sure you want to delete your account?', 'Delete Account', 'Cancel', function() {
        $.ajax({
            ...
            if (!data.success)
                ...
                window.alert('An account deletion confirmation request has been sent to the email address registered to your account.');
        }).fail(function(xhr, text_status, error_thrown) {
            window.alert('An unknown error occurred.');
        });
    });
...
function view_upp_url(index, upp_id) {
  var parent_tr = $('#upp_tr_' + index);
  if (!$('#custom_part_' + index).is(':visible')) {
    if (parent_tr.hasClass('upp-last')) 
      parent_tr.removeClass('last');
...
function show_upp_dialog(){
    ...
    if (false) {
        $('#upp_dialog_auto_fetch input[type=text]').val('');
        $('#upp_dialog_auto_fetch').show();
        $('#upp_dialog_auto_fetch_spinner').hide();
        $('#upp_dialog_auto_fetch_result').html('').hide();
        $('#upp_dialog_auto_fetch_error').html('').hide();
        $('#upp_dialog_manual_fetch').show();
        $('#upp_dialog_manual_entry_form').hide();
        $('#upp_dialog_auto_fetch input[type=submit]').show();
        $('#upp_dialog_auto_fetch input[type=text]').prop('disabled', false);
    }
    else {
        $('#upp_dialog_auto_fetch input[type=text]').val('');
        $('#upp_dialog_auto_fetch').hide();
        $('#upp_dialog_auto_fetch_spinner').hide();
        $('#upp_dialog_auto_fetch_result').html('').hide();
        $('#upp_dialog_auto_fetch_error').html('').hide();
        $('#upp_dialog_manual_fetch').hide();
        $('#upp_dialog_manual_entry_form').show();
        $('#upp_dialog_auto_fetch input[type=submit]').show();
        $('#upp_dialog_auto_fetch input[type=text]').prop('disabled', false);
    }

    $('#upp_dialog').dialog('option', 'title', 'Add Custom Part');
    $('#upp_dialog').dialog('open');
}
...
function show_upp_edit_dialog(upp_id, upp_items){
    for (var upp_i in upp_items) {
      ...
      if (upp_d.id == upp_id) {
        ...
        $('#upp_dialog').dialog('option', 'title', 'Edit Custom Part');
        $('#upp_dialog').dialog('open');
...
function init_partlist_custom_dialog(upp_items) {
    ...
    $('#add_custom_own').change(function() {
    ...
    $('a.delete-custom').click(function(){
      $('#f' + $(this).attr('id')).submit();
      return false;
    });
    $('a.edit-custom').click(function(){
      var upp_id = parseInt($(this).attr('id').substr('custom_edit_'.length));
      show_upp_edit_dialog(upp_id, upp_items);
      return false;
    });

    $('#upp_add_custom_form').submit(function(e){
        var form_data = $(this).serialize();
        e.preventDefault();
        $('#upp_dialog_auto_fetch input[type=submit]').hide();
        $('#upp_dialog_auto_fetch input[type=text]').prop('disabled', true);
        $('#upp_dialog_auto_fetch_spinner').show();
        $('#upp_dialog_manual_fetch').hide();
        ...
        pp_post_wait('/qapi/upp/lookup/', form_data, function(data) {
            ...
            $('#upp_dialog_auto_fetch').hide();
            $('#upp_dialog_auto_fetch_error').html('').hide();
            $('#upp_dialog_auto_fetch_result').html(data.content).show();
        }, function(error) {
            $('#upp_dialog_auto_fetch_spinner').hide();
            $('#upp_dialog_auto_fetch input[type=submit]').show();
            $('#upp_dialog_auto_fetch input[type=text]').prop('disabled', false);
            $('#upp_dialog_auto_fetch_result').html('').hide();
            $('#upp_dialog_auto_fetch_error').html(error).show();
            $('#upp_dialog_manual_fetch').show();
        });
    });
    $('#upp_dialog_manual_fetch input.upp_manual_enter').click(function(){
        $('#upp_dialog_manual_fetch').hide();
        $('#upp_dialog_manual_entry_form').show();
        $('#upp_dialog_auto_fetch_result').hide();
        $('#upp_dialog_auto_fetch_error').hide();
        $('#upp_dialog_auto_fetch').hide();
    });
    $('#upp_dialog').on('click', 'input.upp_dialog_close', function(){
      $('#upp_dialog').dialog('close');
      return false;
    });
}
...
function pp_partlist_add_from_inventory(xid) { pp_common_ajax_redirect('/qapi/partlist/add_from_inventory/', { xid: xid }); }

function pp_reorder_guide_lists(permalink_tags) {
    pp_reorder_partlists_common('/qapi/user/guide/reorder/', permalink_tags);
}
function pp_reorder_userbuild_lists(permalink_tags) {
    pp_reorder_partlists_common('/qapi/user/build/reorder/', permalink_tags);
}
function pp_reorder_userbuild_images(userbuild_id, image_ids) {
	$.ajax({
		type: "POST",
		url: '/qapi/user/build/image/reorder/',
		data: {
...
function pp_change_userbuild_partlist(userbuild_tag, partlist_tag) {
    pp_common_ajax_redirect('/builds/edit/partlist/submit/', {
        userbuild: userbuild_tag,
        partlist: partlist_tag 
    });
}



/* Code below Copyright 2013-2014 PCPartPicker, LLC */
function parse_name_column(s) {
	var n = s.indexOf('<a href');
	if (n > 0) s = s.substr(n+1);
	n = s.indexOf('>');
	if (n > 0) s = s.substr(n+1);
	return s;
}
function parse_mem(s) { return parse_memcore(s, 1024); }
function parse_memi(s) { return parse_memcore(s, 1000); }
function parse_memcore(s, n) {
	if (s.length <= 2) return 0;
	if (s == 'N/A' || s == '') return 0;
	var t = s.charAt(s.length - 2).toLowerCase();
	if (t == 'p') return parseInt(parseFloat(s)*n*n*n*n*n);
	if (t == 't') return parseInt(parseFloat(s)*n*n*n*n);
	if (t == 'g') return parseInt(parseFloat(s)*n*n*n);
	if (t == 'm') return parseInt(parseFloat(s)*n*n);
	if (t == 'k') return parseInt(parseFloat(s)*n);
	return parseInt(s);
}
function parse_rpm(s) { if (s == 'SSD') return 0; if (s == 'Hybrid') return 1; return parseInt(s); }
function parse_modules(s) { var ss = s.toLowerCase().split('x'); return parseInt(ss[0])*1000000000000 + parse_mem(ss[1]); }
function parse_resolution(s) { var ss = s.toLowerCase().split('x'); return parseInt(ss[0])*100000 + parseInt(ss[1]); }
function parse_aspect_ratio(s) { var ss = s.toLowerCase().split(':'); return parseInt(ss[0])*100000 + parseInt(ss[1]); }
function parse_numstr(s) { var r = parseFloat(s); if (isNaN(r)) return 99999999; return r; }
function parse_numstrinv(s) { var r = parseFloat(s); if (isNaN(r)) return -99999999; return r; }
function parse_price(s) { if (s == 'N/A' || s == '') return 999999999; return parseFloat(s.substring(1)); }
function parse_wattage(s) { if (s == 'None') return 0; return parseInt(s); }
function parse_clock_rate(s) {
	if (s.length <= 2) return 0;
	if (s == 'N/A' || s == '') return 0;
	var val = parseFloat(s);
	var t = s.charAt(s.length - 3).toLowerCase();
	if (t == 'g') return parseInt(val*1000*1000*1000);
	if (t == 'm') return parseInt(val*1000*1000);
	if (t == 'k') return parseInt(val*1000);
	return parseInt(val);
}
function parse_optical(s) {
	var n = 0.0;
...
function ParametricPogoStick()
{
	var self = this;
    var selection_num = 0;
	var selection = {};
	var unselected_filter_criteria = [];
	var unselected_show_set = null;
	var unselected_low_price = -1;
	$('#compare_selected').click(function(e){
		...
		for (var i in selection)
			pbids.push(i);
		window.location = '/qapi/parts/compare/by_id/?ids=' + pbids.join(',');
		return false;
	});
	$('#clear_selected').click(function(e){
		if ($(this).hasClass('disabled')) {
			e.preventDefault();
			return false;
		}
	...
	$('#parametric_add_button').click(function(){
		$('#parametric_add_filter').val(window.location.hash);
		var xx = [];
		for (var k in selection) xx.push(k);
		$('#parametric_add_filter_ids').val(xx.join(','));
		$('#parametric_add_form').submit();
		return false;
	});
	this.toggle_selection = function(data) {
		if (selection[data.id]) {
			delete selection[data.id];
            selection_num--;
        }
		else {
			selection[data.id] = data;
            selection_num++;
        }
		self._update_content();
	}
	...
	this.update_hash = function() {
		$('#parametric_price_alert_filter_value').val(window.location.hash);
	}
	this.set_low_price = function(low_price) {
		unselected_low_price = low_price;
		self._update_content();
	}
	this.update_filter_criteria = function(filter_string) {
		unselected_filter_criteria = filter_string;
		self._update_content();
	}
	this._update_content = function() {
		...
		if (selection_num == 0) {
			...
			low_price = unselected_low_price;
		}
		else {
			var base_names = [];
			var base_ids = [];
			for (var k in selection) {
				base_ids.push(k);
				base_names.push(selection[k].name);
				var selection_price = selection[k].price;
		...
		$('#parametric_add_filter_ids').val(base_ids_str);
		$('#parametric_price_alert_filter_ids').val(base_ids_str);
		if (low_price > 0)
...
function init_paging_scroller(container) {
    $('body').on('click', 'div.page-button a', function() {
      filter_set_page($(this).text());
      if ($('body').scrollTop() > $(container).offset().top)
        $('body').animate({ scrollTop: $(container).offset().top }, 500);
      return false;
    });
}

function init_price_settings() {
	$('input.price_settings').change(function(){
	  var checked = $(this).is(':checked') ? 1 : 0;
	  $.post("/qapi/pricesettings/set/", { setting: $(this).attr('id'), value: checked }, function(data) {
		window.location.reload();
	  }, "json");
	});
	$('#price_settings_local_location_none_link').click(function(){
	  $('#price_settings_local_location_none').hide();
	  $('#price_settings_local_location_edit').show();
	  return false;
	});
	$('#price_settings_local_location_edit_link').click(function(){
	  $('#price_settings_local_location_show').hide();
	  $('#price_settings_local_location_edit').show();
	  return false;
	});
	$('#price_settings_local_location_update').click(function(){
	  var zipcode = $("#price_settings_local_pickup_zipcode").val();
	  var radius = $("#price_settings_local_pickup_radius").val();
	  $.post("/qapi/pricesettings/set/", { setting: "price_settings_local_only_location", zipcode: zipcode, radius: radius }, function(data) {
		window.location.reload();
	  }, "json");
	});
	$('#price_settings_show_merchants_link').click(function(){
	  $('#price_settings_show_merchants').hide();
	  $('#price_settings_merchant_list').show();
	  $('#price_settings_local_only').show();
	  return false;
	});
}
...
function init_country_selector(current) {
	$("#country_select").change(function(){
		var ccabbr = $("#country_select").val();
		if (ccabbr != current) {
			$('#form_country_select_url').val(window.location.pathname + window.location.search + window.location.hash);
			$('#form_country_select').submit();
		}
	});
}
...
function xx_toggle_help(parent_id) {
	...
	if (!helpdiv.is(':visible')) {
		helpdiv.html('<table class="comment-help-table">'
			...
			+ '<tbody><tr><td>*italics*</td><td><em>italics</em></td></tr><tr><td>**bold**</td><td><b>bold</b></td></tr>'
			...
			+ '<tr><td>Lines starting with four spaces<br/>are treated like code:<br/><br/><span class="spaces">&nbsp;&nbsp;&nbsp;&nbsp;</span>if 1 * 2 &lt 3:<br/><span class="spaces">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>print "hello, world!"<br/></td><td>Lines starting with four spaces<br/>are treated like code:<br/><pre>if 1 * 2 &lt 3:<br/>&nbsp;&nbsp;&nbsp;&nbsp;print "hello, world!"</pre></td></tr>'
...
function cx_make_comment_edit_form(root_id, parent_id, comment_id, text) {
	s = '<form class="comment_form">'
		+ '<div class="comment-edit-form">'
			...
			+ '<input type="submit" value="Save" onclick="cx_submit_edit(this,' + comment_id + ');return false;">'
			+ '<input type="button" value="Cancel" onclick="cx_edit_toggle(' + root_id + ',' + parent_id + ',' + comment_id + ');return false;">'
			...
			+ '<div id="cx' + comment_id + '_form_error" class="comment-form-error">There was an error submitting your comment.</div>'
			+ '<div id="cx' + comment_id + '_help_box" class="comment-help-box"></div>'
		+ '</div>'
	+ '</form>';
	return s;
}
function cx_make_comment_reply_form(root_id, parent_id, permanent) {
	s = '<form class="comment_form">'
		+ '<div class="comment-edit-form">'
			...
			+ '<input type="submit" value="Save" onclick="cx_submit_reply(this,' + parent_id + ',' + permanent + ');return false;">';
	if (!permanent)
		s += '<input type="button" value="Cancel" onclick="cx_reply_toggle(' + root_id + ',' + parent_id + ',' + permanent + ');return false;">';
	s += '<a href="#" class="comment-help-link" id="cx' + parent_id + '_help" onclick="xx_toggle_help(\'cx' + parent_id + '\');return false;">Formatting Help</a>'
		...
		+ '<div id="cx' + parent_id + '_help_box" class="comment-help-box"></div>'
		+ '</div>'
	+ '</form>';
	return s;
}
function mx_make_comment_reply_form(message_id) {
	s = '<form class="comment_form">'
		+ '<div class="comment-edit-form">'
			...
			+ '<input type="submit" value="Save" onclick="mx_submit_reply(this,' + message_id + ');return false;">'
			+ '<input type="button" value="Cancel" onclick="mx_reply_toggle(' + message_id + ');return false;">'
			...
			+ '<div id="mx' + message_id + '_help_box" class="comment-help-box""></div>';
		+ '</div>'
	+ '</form>'
	return s;
}

function init_common_buttons() {
	$('body').on('click', 'a.pp_add_part', function () { pp_partlist_add($(this).attr('href').substr(1), 1); return false; });
}
...
function run_confirm_dialog(title, message, ok_msg, cancel_msg, callback, nopad, width, validate_callback) {
	var dlg = $('#default_dialog');
	if (nopad == undefined || !nopad)
		message = '<div style="text-align: center;padding:30px;">' + message + '</div>';
	else
		message = '<div>' + message + '</div>';
	dlg.html(message + '<div style="white-space:nowrap;text-align: center;"><input type="button" value="' + ok_msg + '" id="dialog_ok" style="margin-right:20px;"/><input type="button" value="' + cancel_msg + '" id="dialog_cancel" /></div>');
	...
	$('#dialog_cancel').click(function () { dlg.dialog('close'); });
	dlg.dialog('open');
}

function run_ok_dialog(title, message, callback) {
	var dlg = $('#default_dialog');
	dlg.html('<div style="text-align: center;padding:30px;">' + message + '</div><div style="white-space:nowrap;text-align: center;"><input type="button" value="OK" id="dialog_ok"/></div>');
	dlg.dialog('option', 'title', title);
	$('#dialog_ok').click(function() { dlg.dialog('close'); if (callback) { callback(); }});
	dlg.dialog('open');
}

function run_report_dialog(linkage_type, linkage_id, linkage_name, reasons) {
	var s = '<div id="desktop_report_dialog"><div class="moderation-report-reasons">Reason:</div><ul>';
	var reasons = moderation_get_reasons(linkage_type);
	for (var i=0; i<reasons.length; ++i) {
		var r = reasons[i];
		s += '<li class="moderation-report-reason"><input type="radio" name="report_reason" value="' + r['value'] + '" id="report_reason_' + r['value'] + '"><label for="report_reason_' + r['value'] + '">' + r['name'] + '</label></li>';
	}
	s += '</ul><div class="moderation-report-no-reason-given">Please choose a reason.</div><div class="moderation-report-additional-info">Additional Information: (optional)</div><textarea rows="4" name="report_message" class="moderation-report-additional-info-text" id="report_message"></textarea></div>';
	run_confirm_dialog('Report ' + linkage_name + '?', s, 'Report', 'Cancel', function() {
		...
		pp_report_object(linkage_type, linkage_id, reason, message, function() {
...
function PartReview(container, link_container, userbuild_id, tag, base_rating, base_description, base_description_rendered, unique_id) {
	this.editable = false;
	this.rating = base_rating;
	this.review_description = base_description;
	this.review_description_rendered = base_description_rendered;
	this.prev_rating = base_rating;
	this.prev_review_description = base_description;
	$(container + ' textarea').val(base_description);
	$(container + ' .part-review-description-content').html(base_description_rendered);
	var n = base_rating;
	var s = '<div class="inline-rating-md"><div class="rating"><ul class="stars">';
	for (var i = 0; i < 5; i++) {
		if (n > 0) {
			s += '<li class="full-star" id="partrate_' + unique_id + '_' + i + '"></li>';
			n--;
		}
		else {
			s += '<li class="empty-star" id="partrate_' + unique_id + '_' + i + '"></li>';
		}
	}
	s += '</ul></div></div>';
	$(container + ' div.rating-box').html(s);

	function make_set_function(rb, i) {
		return function () {
			if (rb.editable) {
				rb.rating = i + 1;
				rb.draw_rating(rb.rating);
			}
		}
	}
	function make_show_function(rb, i) {
		return function () {
			if (rb.editable) {
				rb.draw_rating(i + 1);
			}
	...
	function make_reset_function(rb) {
		return function () {
			if (rb.editable) {
				rb.draw_rating(rb.rating);
			}
	...
	this.buttons = [];
	for (var i = 0; i < 5; ++i) {
		var button = $('#partrate_' + unique_id + '_' + i);
		this.buttons.push(button);
		button.click(make_set_function(this, i));
		button.hover(make_show_function(this, i), make_reset_function(this));
	}
	function make_send_function(rb) {
	  return function() {
		var review_description = $(container + ' textarea');
		$.ajax({
		  type: "POST",
		  url: '/qapi/product/rate/',
		  data: {
			userbuild_id: userbuild_id,
			tag: tag,
			rating: rb.rating,
		  ...
		  dataType: "json"
			...
			if (data.success) {
				rb.editable = false;
				rb.prev_rating = data.rating;
				rb.rating = data.rating;
				rb.prev_review_description = data.review_description;
				rb.review_description = data.review_description;
				rb.review_description_rendered = data.review_description_rendered;
				rb.update_action_string();
				rb.mark_not_editable();
			}
			else {
	...
	function make_cancel_function(rb) {
	  return function() {
		var review_description = $(container + ' textarea');
		review_description.val(rb.prev_review_description);
		rb.mark_not_editable();
		return false;
	  }
	};
	...
	$(container + ' input[type="submit"]').click(make_send_function(this));
	$(container + ' input[type="button"]').click(make_cancel_function(this));
	this.draw_rating = function(rating) {
		var r_off = 'empty-star';
		var r_on = 'full-star';
		for (var i = 0; i < 5; i++) {
			if (rating > i)
				this.buttons[i].addClass(r_on).removeClass(r_off);
			else
				this.buttons[i].addClass(r_off).removeClass(r_on);
		}
	}
	this.update_action_string = function() {
	  if (this.rating != 0 || this.review_description.length)
		$(link_container).text('Edit review');
	  else
		$(link_container).text('Add review');
	}
	this.open = function() {
		$(container).show();
		$(container + ' .part-review-description-content').hide();
        $(container + ' .part-review-submit-error').text('');
		$(container + ' .part-review-submit-row').show();
		$(container + ' textarea').show();
		$(container + ' .rating-box li').addClass('part-review-editable');
		this.editable = true;
	}
	this.mark_not_editable = function() {
		this.editable = false;
		$(container + ' textarea').val(this.prev_review_description);
		$(container + ' .part-review-description-content').html(this.review_description_rendered);
		$(container + ' .rating-box li').removeClass('part-review-editable');
		if (this.review_description_rendered)
		  $(container + ' .part-review-description-content').html(this.review_description_rendered);
		else
		  $(container + ' .part-review-description-content').html('(You have not entered a review description.)');
		var rcc = $(container + ' .part-review-description-content');
		if (this.rating == 0 && this.review_description.length == 0) {
			$(container).hide();
		}
		else {
			$(container).show();
			$(container + ' .part-review-description-content').show();
			$(container + ' textarea').hide();
			$(container + ' .part-review-submit-row').hide();
		}
	}

	this.draw_rating(this.rating);
	this.update_action_string();
	this.mark_not_editable();
	$(link_container).click(make_open_function(this));
}
...
function delete_saved_confirm(save_id) {
	run_confirm_dialog('Confirm Saved Part List Deletion', 'Are you sure you want to delete this saved part list?', 'Delete Saved Part List', 'Cancel', function() {
		pp_common_ajax_redirect('/qapi/user/saved/delete/', { save_id: save_id });
	});
}
...
function delete_userbuild_confirm(userbuild_id) {
	run_confirm_dialog('Confirm Completed Build Deletion', 'Are you sure you want to delete this completed build?', 'Delete Completed Build', 'Cancel', function() {
		pp_common_ajax_redirect('/qapi/user/build/delete/', { userbuild_id: userbuild_id });
	});
}

function delete_guide_confirm(guide_id) {
	run_confirm_dialog('Confirm Build Guide Deletion', 'Are you sure you want to delete this build guide?', 'Delete Build Guide', 'Cancel', function() {
		pp_common_ajax_redirect('/qapi/user/guide/delete/', { guide_id: guide_id });
	});
}

function init_carousel(container, num_items, additionalOptions) {
	var defaultOptions = {
		items: num_items,
		margin: 10,
		responsive: false,
...
function show_gallery(idx, carousel_images) {
  $(document).lightGallery({
	...
	counter: true,
	lang: { allPhotos : 'All images' },
	dynamic: true,
	dynamicEl: carousel_images
  });
}
...
function stop_loading_animation(arg) {
    if (loading_animation_depth > 0)
        loading_animation_depth -= 1;
    if (loading_animation_depth == 0) {
...
function init_partlist_price_dialog(obj_data) {
    $('#price_dialog').dialog({
      autoOpen: false,
      title: 'Configure Part Price',
      width: 500,
      modal: true,
      resizable: false
    });
    $('a.change-price').click(function(){
      var obj_index = parseInt($(this).attr('id').substr('change_price_'.length));
      show_price_dialog(obj_index);
      return false;
    });
    $('#change_price_cancel').click(function(){ $('#price_dialog').dialog('close'); });
    ...
    function _set_price_dialog_enables(mode) {
      if (mode == 'manual') {
        $('.price-manual-options input').removeAttr('disabled');
      ...
      for (var i in obj_data) {
        ...
        if (obj_d.index == obj_index) {
          ...
          if (obj_d.inventory.length == 0) {
            $('#change_price_inventory').prop('disabled', (obj_d.inventory.length == 0));
...
function init_partlist_history_dialog() {
    $('#change_history_dialog').dialog({
      autoOpen: false,
      title: 'Part List History',
      width: 800,
      modal: true,
      resizable: false
    });
    $('#change_history').click(function(){
      $('#change_history_dialog').dialog('open');
      $('#change_history_dialog a').blur();
      return false;
    });
}
...
function init_partlist_markup_dialog(permalink_tag, parametric_tags) {
    $('#markup_dialog').dialog({
      autoOpen: false,
      title: 'Export Part List — Copy And Paste Into Your Post',
      width: 600,
      modal: true,
      resizable: false
    });
    function open_markup(t) {
      $('#markup_text').html('Loading...');
      $('#markup_dialog').dialog('open');
      g_markup_type = t;
      fetch_markup();
    }
    function fetch_markup() {
      ...
      $.getJSON('/qapi/partlist/markup/', request_data, function(data) {
    ...
    $('#markup_single_merchant').change(function() {
      g_markup_merchant = $(this).val();
      fetch_markup();
    });
    $('a#export_markup_markdown').click(function() { open_markup('markdown'); return false; });
    $('a#export_markup_reddit').click(function() { open_markup('markdown'); return false; });
    $('a#export_markup_bbcode').click(function() { open_markup('bbcode'); return false; });
    $('a#export_markup_html').click(function() { open_markup('html'); return false; });
    $('a#export_markup_plaintext').click(function() { open_markup('plaintext'); return false; });
}

function init_partlist_save_dialog(ccabbr, permalink_tag) {
    $('#partlist_save_dialog').dialog({
      autoOpen: false,
      title: 'Save Part List As...',
      width: 600,
      modal: true,
      resizable: false
    });
    $('#partlist_save_button').click(function(){
      $('#partlist_save_dialog').dialog('open');
      $.getJSON('/qapi/partlist/choices/?cc=' + ccabbr + '&partlist=' + permalink_tag, function(data) {
        if (data.success) {
          $('#partlist_save_dialog').html(data.content);
          $('#partlist_save_existing_choice').click(function(){
            var rbutton = $('input:radio[name=save_choice]');
            $(rbutton[0]).prop('checked', false);
            $(rbutton[1]).prop('checked', true);
          });
          $('#partlist_save_new_text').click(function(){
            var rbutton = $('input:radio[name=save_choice]');
          ...
          $('#partlist_save_dialog').html('An error has occurred.');
      });
      return false;
    });
}

function init_partlist_incompatibility_dialog(permalink_tag) {
    $('#flag_incompat_dialog').dialog({
      autoOpen: false,
      title: 'Flag Issues/Incompatibility As Incorrect',
      width: 400,
      modal: true,
      resizable: false
    });
    $('a.flag-incompatibility').click(function(){
      $('#flag_incompat_dialog').dialog('open');
      return false;
    });
    $('#flag_incompat_submit').click(function(){
      $('#flag_incompat_submit').attr('disabled', 'disabled');
      $.post("/qapi/partlist/report_invalid_incompatibility/", { permalink_tag: permalink_tag, comment:$('#flag_incompat_comment').val() });
      $('#flag_incompat_dialog').html('The reported issues/incompatibilites have been flagged as incorrect.  Thanks for letting us know!');
      $('#flag_incompat_callout').hide();
      return false;
    });
    $('#flag_incompat_cancel').click(function(){
      $('#flag_incompat_comment').val('');
      $('#flag_incompat_dialog').dialog('close');
      return false;
    });
}
...
function pp_partlist_replace(identifier, tag) { pp_common_ajax_redirect('/qapi/partlist/replace/', { identifier: identifier, replace_with: tag }); }
function pp_partlist_delete(identifier) { pp_common_ajax_redirect('/qapi/partlist/delete/', { identifier: identifier }); }
function pp_partlist_add(tag, quantity) { pp_common_ajax_redirect('/qapi/partlist/add/', { tag: tag, quantity: quantity }); }
function pp_partlist_add_from_favorite(xid) { pp_common_ajax_redirect('/qapi/partlist/add_from_favorite/', { xid: xid }); }
function pp_partlist_add_from_inventory(xid) { pp_common_ajax_redirect('/qapi/partlist/add_from_inventory/', { xid: xid }); }
function pp_partlist_edit(permalink_tag) { pp_common_ajax_redirect('/qapi/partlist/edit/redirect/', { permalink_tag: permalink_tag }); }
function pp_partlist_delete_all() { pp_common_ajax_redirect('/qapi/partlist/delete/all/redirect/', { }); }
function pp_partlist_delete_all_custom_prices() { pp_common_ajax_redirect('/qapi/partlist/delete/custom_prices/redirect/', { }); }
...
function init_partlist_wattage_dialog() {
    $('#explain_wattage_dialog').dialog({
      autoOpen: false,
      title: 'Wattage Breakdown',
      width: 600,
      modal: true,
      resizable: false
    });
    $('#explain_wattage').click(function(){
      $('#explain_wattage_dialog').dialog('open');
      return false;
    });
}
